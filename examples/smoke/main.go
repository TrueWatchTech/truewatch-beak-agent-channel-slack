...
// beak-agent-channel-slack. It is NOT part of the automated test suite:
// every credential is read from an environment variable, never hard-coded, and
// every call goes over the network to the real Slack API. Use it to prove,
// with a live account, that:
//
//	validate            — the real Slack API accepts the credential and the bot identity resolves
...
// arguments, or see the credential_fields in platforms/slack.yaml.
//
// Slack operator steps:
//  1. Open your app at https://api.slack.com/apps.
//  2. Basic Information > App Credentials > Signing Secret -> BEAK_SIGNING_SECRET.
//  3. OAuth & Permissions > Bot User OAuth Token (xoxb-...) -> BEAK_BOT_TOKEN.
//  4. Run `smoke validate` — proves the token via auth.test.
//  5. Run `smoke serve :8080` and expose it publicly (e.g. `ngrok http 8080`).
//  6. Event Subscriptions > Enable Events > Request URL = https://<public-host>/
//     (this tool answers the url_verification handshake automatically).
//  7. Subscribe to bot events as needed: message.im, message.channels, app_mention.
//  8. DM or @-mention the bot in Slack; watch this tool's stderr log for the
//     verified inbound event.
//  9. Run `smoke send <channel_id> hello` — proves outbound chat.postMessage.
package main
...
var credentialFields = []credentialFieldSpec{
	...
	{Key: "signing_secret", Title: "Signing Secret", Required: true},
}
...
func usage() {
	fmt.Fprintln(os.Stderr, "smoke - manual real-API integration tool for beak-agent-channel-slack")
...
// ValidateCredential, which hits the real Slack API through
...
func runServe(ctx context.Context, connector beak.Connector, runtime sdk.Runtime, account sdk.ChannelAccount, addr string) {
	...
	runtime.Logger.Printf("listening on %s for Slack webhook deliveries (bot=%s)", addr, result.DisplayName)
...
// real Slack API through runtime.HTTPClient.
